...
void lprofP_callhookIN(lprofP_STATE* S, char *func_name, char *file, int linedefined, int currentline,char* what, char* cFun, lprof_DebugInfo* dbg_info) 
{
	...
	if (!S->stack_top && !filter_lua_api(func_name, dbg_info->p_source))//如果是空的，就筛选一下是否要开始记录， 如果不是空的，直接记录
		return;

	
	if (!S->stack_top)//从栈空的开始记录的第一个函数
	{
	...
	if (dbg_info->level > curBaseFuncLevel + iFunFilterLevel)//如果当前函数层数超过要记录的层数，退出
		return;
